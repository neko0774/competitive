  ...
  while(!C.empty()) {
    ...
    for(int i = 1; i < l; i++) {
      ...
      if((min_right != -1 && max_left != -1) && min_right <= max_left) {
        ...
        // top
        color(i, min_right, C[i]);
        color(i, max_left, C[m - i - 1]);
        // right
        color(min_right, m - 1 - i, C[i]);
        color(max_left, m - 1 - i, C[m - 1 - i]);
        // bottom
        color(m - i - 1, m - 1 - min_right, C[i]);
        color(m - i - 1, m - 1 - max_left, C[m - 1 - i]);
        // left
        color(m - 1 - min_right, i, C[i]);
        color(m - 1 - max_left, i, C[m - 1 - i]);
      }
    }
    rebuild();
  }
